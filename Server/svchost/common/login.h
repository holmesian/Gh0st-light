...
typedef BOOL (VFWAPI *capGetDriverDescriptionT)
(
	UINT wDriverIndex,
...
bool IsWebCam()
{
	capGetDriverDescriptionT pcapGetDriverDescription = (capGetDriverDescriptionT)GetProcAddress(LoadLibrary("AVICAP32.dll"),"capGetDriverDescriptionA");
	bool	bRet = false;
...
UINT GetHostRemark(LPCTSTR lpServiceName, LPTSTR lpBuffer, UINT uSize)
{
	...
	char szHost[] = {'H','o','s','t','\0'};
	ReadRegEx(HKEY_LOCAL_MACHINE, strSubKey, szHost, REG_SZ, (char *)lpBuffer, NULL, uSize, 0);
